...
The version directory the launcher selects is the CLI version inlined at build time, so the workflow sets `0.0.40-pi.<short-sha>` before building. The previous version stays installed and the launcher rolls back when a new version fails to start; host details, pairing commands, and the explicit rollback recipe are in `~/local/t3-pi/DEPLOYMENT.md` on that machine.
