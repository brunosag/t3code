...
const makeDesktopEnvironmentLayer = (env: Readonly<Record<string, string | undefined>> = {}) =>
...
function makeTestLayer(input: {
  ...
  readonly desktopEnv?: Readonly<Record<string, string | undefined>>;
  ...
  return DesktopWindow.layer.pipe(
    Layer.provide(
      Layer.mergeAll(
        desktopAssetsLayer,
        input.desktopEnv ? makeDesktopEnvironmentLayer(input.desktopEnv) : desktopEnvironmentLayer,
...
describe("DesktopWindow", () => {
  ...
  it.effect("opens development DevTools when T3CODE_DESKTOP_DEVTOOLS is set", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      const createCount = yield* Ref.make(0);
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
      const layer = makeTestLayer({
        ...
        createCount,
        mainWindow,
        desktopEnv: { T3CODE_DESKTOP_DEVTOOLS: "true" },
      });
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.openDevTools.mock.calls[0], [{ mode: "detach" }]);
      }).pipe(Effect.provide(layer));
    }),
  );
